...
class ReservationStatisticsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        amount_reserved_by_variation_id = {
            ...
            for l in ReservationPosition.objects.exclude(
                part__reservation__state=Reservation.STATE_CANCELLED
            )
            .values("variation")
        ...
        def render_variation_count_in_reservations(variation):
            return '<div class="text-center">{}</span></div>'.format(
                amount_reserved_by_variation_id.get(variation.id, 0)
            )
...
class ReservationAdministrationView(LoginRequiredMixin, View):
    ...
    StateButton = namedtuple(
        "StateButton", ["state", "label", "active_classes", "inactive_classes"]
    )
    state_buttons = [
        StateButton(
            Reservation.STATE_UNAPPROVED,
            "Unapproved",
            "btn-secondary",
            "btn-outline-secondary",
        ),
        StateButton(
            Reservation.STATE_EDITABLE,
            "Editable",
            "btn-secondary",
            "btn-outline-secondary",
        ),
        StateButton(
            Reservation.STATE_SUBMITTED,
            "Submitted",
            "btn-secondary",
            "btn-outline-secondary",
        ),
        StateButton(
            Reservation.STATE_PACKED, "Packed", "btn-secondary", "btn-outline-secondary"
        ),
        StateButton(
            Reservation.STATE_READY, "Ready", "btn-secondary", "btn-outline-secondary"
        ),
        StateButton(
            Reservation.STATE_PICKED_UP,
            "Picked up",
            "btn-success",
            "btn-outline-success",
        ),
        StateButton(
            Reservation.STATE_CANCELLED, "Cancelled", "btn-danger", "btn-outline-danger"
        ),
    ]
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request, messages.ERROR, "This action could not be performed."
            )
...
class ReservationPDFDownloadView(LoginRequiredMixin, View):
    def get(self, request, reservation_id):
        ...
        data = generate_packing_pdf(
            [reservation], filename, username=request.user.username
        )
...
class ReservationPackedActionForm(forms.Form):
    BEHAVIOUR_CHOICES = [
        ("behaviour__set_packed", "Only set reservation to packed"),
        (
            "behaviour__set_ready_for_pickup",
            "Set reservation to packed and ready for pickup",
        ),
    ]

    i_checked_my_action = forms.BooleanField(
        label="I know what I'm doing and checked my actions"
    )
    requested_behaviour = forms.ChoiceField(
        ...
        if f.is_valid():
            if f.cleaned_data["i_checked_my_action"]:
                ...
                if f.cleaned_data["requested_behaviour"] == "behaviour__set_packed":
                    ...
                    f.cleaned_data["requested_behaviour"]
                    == "behaviour__set_ready_for_pickup"
                ):
