...
class ReservationPartDeleteView(View):
    ...
    def post(self, request, secret, part_id):
        ...
        if form.is_valid():
            part.delete()
            messages.add_message(self.request, messages.SUCCESS, f"{part!s} has been deleted.")
            return redirect("reservationdetail", secret=secret)
    ...
    def post(self, request, secret):
        ...
        if form.is_valid():
            new_part = ReservationPart.objects.create(
                reservation=reservation,
                title=f"{reservation.team_name} #{number_of_parts + 1}",
            )
...
def amount_reserved(variation):
    return (
        ReservationPosition.objects.filter(variation=variation).aggregate(reserved=Sum("amount"))[
            "reserved"
        ]
        or 0
    )
...
class ReservationPartDetailView(View):
    ...
    def post(self, request, secret, part_id):
        ...
        messages.add_message(
            self.request,
            messages.SUCCESS,
            f"Part {reservation_part.title} has been saved.",
        )
        return redirect("reservationdetail", secret=secret)
