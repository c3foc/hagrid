...
def timestamp(filestr=True):
    timestring: str = timezone.now().strftime("%Y-%m-%d %H:%M:%S")
    if filestr:
...
def generate_collection_list(reservation: Reservation, distinct_required=False):
    ...
    if distinct_required:
        for part in reservation.parts.all():
            ...
            titles.append(f"packing list for part {part.title!s}:")
    else:
...
def render_invoice_header(r: Reservation, d: Document):
    ...
    if textheight + 25 > 160:
        ...
        d.cursor_y = min(d.cursor_y, d.h - 180)
    d.canvas.setFont("Helvetica", 11)
...
def render_collection_list_entry(pos: SizeVariation, amount: int, d: Document):
    ...
    d.canvas.drawString(
        d.cursor_x + 460 - d.get_text_width(str(amount), text_size=11), d.cursor_y - 10, str(amount)
    )
    d.cursor_y -= 15
...
def render_invoice_end(l, d: Document):
    ...
    d.canvas.drawString(d.w - 175, cy - 10, f"{total:20,.2f} €")
...
def get_side_stip_text(r: Reservation):
    return f"Reservation ID: {r.id}, Reservation state: {r.state!s}, Printed at {timestamp()}"
...
def generate_packing_pdf(
    ...
    if len(reservations) < 1:
        raise ValueError("There needs to be at least one reservation to export.")
    logger.debug("Exporting reservations: " + str(reservations))
    d: Document = Document(
        filename,
        title,
        f"The robots in slavery by {username!s}.",
        f"This document, originally created at {timestamp(filestr=False)}, contains the requested reservations.",
