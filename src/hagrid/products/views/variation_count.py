...
def variation_count(request, code, variation_id=None):
        ...
        if not variation_id:
            ...
            available_variations = access_code.variations.filter(
                ...
                & (
                    ...
                    | Q(count_reserved_until__lt=timezone.now())
                )
                & (
                    Q(count_disabled_until__isnull=True)
                    | Q(count_disabled_until__lt=timezone.now())
                )
            )
            if not available_variations:
                ...
                if request.POST and form.is_valid():
                    ...
                    variation.count_reserved_until = timezone.now() + timedelta(minutes=15)
                    variation.save()

                    # assign a variation and redirect
                    return redirect("variation_count", code, variation.id)

                important = sum((int(p["total"] >= 0.5) for p in priorities), 0)
    ...
    if request.POST:
        ...
        if common_form.is_valid() and common_form.cleaned_data["action"] != "save":
        ...
        elif common_form.is_valid() and all(i["form"].is_valid() for i in items):
            total = 0
            items_changed = 0
            ...
            if access_code.as_queue:
                ...
                return redirect("variation_count", code)
            return redirect(
                reverse("variation_count_success") + f"?total={total}&items_changed={items_changed}"
            )
    ...
    if request.POST:
        for priority in priorities:
            ...
            if form.is_valid() and variation.id == form.cleaned_data["variation"]:
                ...
                messages.info(
                    request,
                    str(variation) + (" bumped" if variation.count_prio_bumped else " unbumped"),
                )
