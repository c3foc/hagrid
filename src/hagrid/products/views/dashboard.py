...
def dashboard(request):
    ...
    def _transform_sizegroup(sizegroup, product_variations):
        variations = (
            product_variations.filter(size__group=sizegroup).order_by("size__position").distinct()
        )
    ...
    def _transform_product_group(product_group):
        ...
        return {
            ...
            "products": [_transform_product(p) for p in products],
        }

    product_availabilities = [_transform_product_group(pg) for pg in product_groups]
...
def dashboard_table(request):
    ...
    context = {
        ...
        "variations": Variation.objects.all(),
        "availability_tables": [
            SizeTable(sg, render_variation=render_variation) for sg in SizeGroup.objects.all()
        ],
    }
